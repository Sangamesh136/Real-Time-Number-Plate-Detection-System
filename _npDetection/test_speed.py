...
class SpeedEstimator:
    ...
    def estimate_speed(self, im0, tracks, original_frame):
        ...
        for box, t_id, cls in zip(boxes, t_ids, clss):
            ...
            # Move the image and annotations to GPU (if possible)
            im0 = torch.tensor(im0).to(self.device)
            annotator.box_label(box, speed_label, bbox_color)
            cv2.polylines(im0.cpu().numpy(), [trk_pts], isClosed=False, color=bbox_color, thickness=self.tf)
            cv2.circle(im0.cpu().numpy(), (int(track[-1][0]), int(track[-1][1])), self.tf * 2, bbox_color, -1)

            if not self.reg_pts[0][0] < track[-1][0] < self.reg_pts[1][0]:
                return im0.cpu().numpy()
        ...
        if self.view_img and self.env_check:
            cv2.imshow("Ultralytics Speed Estimation", im0.cpu().numpy())
            if cv2.waitKey(1) & 0xFF == ord("q"):
                return
